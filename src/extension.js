...
function analyzeSourceUnit(cancellationToken, document){
    ...
    for (var contract in insights.contracts) {
        ...
        for (var stateVar in insights.contracts[contract].stateVars) {
            ...
            let decl_uri = "([Declaration: #"+(svar.loc.start.line)+"]("+activeEditor.document.uri+"#"+(svar.loc.start.line)+"))";
            ...
            insights.contracts[contract].functions[functionName].identifiers.forEach(ident => {
                ...
                if(is_declared_locally){
                    ...
                    if(is_state_var){
                        ...
                        let decl_uri = "([Declaration: #"+(ident.loc.start.line)+"]("+activeEditor.document.uri+"#"+(ident.loc.start.line)+"))";
...
function onActivate(context) {
    ...
    async function registerDocType(type, docSel) {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand("solidity-va.etherscan.getCode", async (args) => {
                if(!args) { 
                    return;
                }
                switch(args.type){
                    case 'sourceCode': 
                        EtherScanIO.sourceCodeForAddress(args.address).then(results => {
                            if(!results || !Array.isArray(results)){
                                vscode.window.showWarningMessage(`etherscan.io did not return a result. Please try again later.`)
                                return;
                            }
                            ...
                            results.forEach(result => {
                                ...
                                if(!sourceCode){
                                    vscode.window.showErrorMessage(`${args.address} has not been verified on etherscan.io yet. Source code unavailable. Try decompiling instead.`)
                                    return;
                                }
 ...
 * 
 * Name: ${result.ContractName} (${args.address})
 * 
...
`;
                                ...
                                vscode.workspace.openTextDocument({content: header + sourceCode, language: "solidity"})
                    ...
                    case 'byteCode': 
                        EtherScanIO.byteCodeForAddress(args.address).then(content => {
                            vscode.workspace.openTextDocument({content: content, language: "evm"})
                                .then(doc => vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside));
                        });
                    break;

...
${Object.values(contractObj.usingFor).reduce((txt, astNode) => {
  return (
    txt +
    `\t{abstract}📚${astNode.libraryName} for [[${
      mod_symbols.getVariableDeclarationType(astNode) || "*"
    }]]\n`
  );
}, "")}
    ' -- vars --
${Object.values(contractObj.stateVars).reduce((umlSvarTxt, astNode) => {
  return (
    umlSvarTxt +
    `\t${variableVisibility[astNode.visibility] || ""}${
      astNode.isDeclaredConst ? "{static}" : ""
    }[[${mod_symbols
      .getVariableDeclarationType(astNode)
      .replace(/\(/g, "")
      .replace(/\)/g, "")}]] ${astNode.name}\n`
  );
}, "")}
    ' -- methods --
${contractObj.functions.reduce((umlFuncTxt, funcObj) => {
  return (
    umlFuncTxt +
    `\t${functionVisibility[funcObj._node.visibility] || ""}${
      ...
      contractObjects.reduce((umlTxt, contractObj) => {
        return (
          ...
          Object.values(contractObj.usingFor).reduce((txt, astNode) => {
            return (
              txt +
              `${contractObj.name} ..[#DarkOliveGreen]|> ${
                astNode.libraryName
              } : //for ${
                mod_symbols.getVariableDeclarationType(astNode) || "*"
              }//\n`

        ...
        .then((insights) => {
          ...
          for (var contractName in insights.contracts) {
            ...
            for (let funcObj of insights.contracts[contractName].functions) {
              ...
              if (numModifiers !== 0) {
                ...
                for (let modifierName in funcObj.modifiers) {
                  ...
                  modifiersLevelNode.children.push(
                    astNodeAsDocumentSymbol(
                      document,
                      vardec,
                      symbolAnnotation.symbol,
              ...
              if (numAssemblyFuncDefs !== 0) {
                ...
                for (let modifierName in funcObj.assemblyFunctions) {
                  ...
                  assemblyLevelNode.children.push(
                    astNodeAsDocumentSymbol(
                      document,
                      vardec,
                      symbolAnnotation.symbol,
            ...
            for (let functionName in insights.contracts[contractName]
              ...
              topLevelNode.children.push(functionLevelNode);
              //get all declarations in function
              for (let declaration in insights.contracts[contractName]
                .modifiers[functionName].declarations) {
                let vardec =
                  insights.contracts[contractName].modifiers[functionName]
            ...
            if (settings.extensionConfig().outline.inheritance.show) {
              ...
              let contractCache = {};
              let contractNodes = {};
              for (var name in insights.contracts[contractName]
                .inherited_names) {
                //skip self
                let inheritedFromContract =
                  insights.contracts[contractName].inherited_names[name];
                let inheritedFromContractName = inheritedFromContract.name;
                if (inheritedFromContractName == contractName) continue;

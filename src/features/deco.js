...
async function decorateSourceUnit(document, editor, sourceUnit) {
  ...
  if (config.deco.warn.externalCalls) {
    decorations.push(
      ...sourceUnit
        .getExternalCalls()
        .map((c) => CreateDecoStyle.extCall(c._node)),
    );
  }
  ...
  if (config.deco.statevars) {
    for (let contract of Object.values(sourceUnit.contracts)) {
      for (let svar of Object.values(contract.stateVars)) {
        decorations.push(
          CreateDecoStyle.stateVarDecl(svar, document, contract),
        );
        ...
        svar.extra.usedAt.forEach((ident) => {
          if (ident.extra.inFunction.declarations[ident.name]) {
            ...
            decorations.push(
              CreateDecoStyle.shadowedStateVar(ident, document, contract, svar),
            );
            let declaration = ident.extra.inFunction.declarations[ident.name];
            decorations.push(
              CreateDecoStyle.shadowedStateVar(
                declaration,
                document,
                contract,
                svar,
            ...
            decorations.push(
              CreateDecoStyle.stateVarIdent(ident, document, contract, svar),
            );
          }
        });
      }
  ...
  identifiers.forEach((ident) => {
    ...
    if (is_declared_locally && !is_inherited && !is_state_var) {
      if (ident.extra.scope === "argument" || ident.extra.scope === "super") {
        ...
        decorations.push(
          CreateDecoStyle.shadowedInheritedStateVar(ident, document, contract),
        );
      }
    } else if (
      (is_declared_locally && is_inherited) ||
      (is_state_var && is_inherited)
    ) {
      decorations.push(
        CreateDecoStyle.shadowedInheritedStateVar(ident, document, contract),
      );
      ...
      decorations.push(
        CreateDecoStyle.inheritedStateVar(ident, document, contract),
      );
    }
  });
...
async function decorateWords(editor, words, decoStyle, commentMapper) {
  ...
  words.forEach(function (word) {
    ...
    while ((match = regEx.exec(text))) {
      if (
        commentMapper &&
        commentMapper.indexIsInComment(
          match.index,
          match.index + match[0].trim().length,
        )
      ) {
        continue;
      }
  ...
  Object.values(funcNode.arguments).forEach((ident, index) => {
    ...
    decorations.push({
      range: new vscode.Range(
        ...
        new vscode.Position(
          ident.loc.end.line - 1,
          ident.loc.end.column + ident.name.length,
        ),
      ),
...
class CreateDecoStyle {
  static reserved(node) {
    ...
    let decl_uri =
      "[more info..](https://solidity.readthedocs.io/en/latest/miscellaneous.html#reserved-keywords)";
    ...
    return {
      range: new vscode.Range(
        ...
        new vscode.Position(
          node.loc.end.line - 1,
          node.loc.end.column + node.name.length,
        ),
      ),
      ...
      range: new vscode.Range(
        ...
        new vscode.Position(
          ...
          node.loc.start.column +
            mod_parser.parserHelpers.getAstNodeName(node.expression).length,
        ), //only highlight first if extcall spans multiple lines
      ),
  ...
  static stateVarDecl(node, document, contract) {
    ...
    return {
      range: new vscode.Range(
        new vscode.Position(
          node.identifier.loc.start.line - 1,
          node.identifier.loc.start.column,
        ),
        new vscode.Position(
          node.identifier.loc.end.line - 1,
          node.identifier.loc.end.column + node.identifier.name.length,
        ),
      ),
      hoverMessage: `${prefix}(*${
      ...
      }*) StateVar *${contract.name}*.**${node.identifier.name}**`,
      decoStyle: decoStyle,
    };
  }
  static stateVarIdent(node, document, contract, svar) {
    ...
    return {
      ...
      hoverMessage: `${prefix}(*${
        svar.typeName.type == "ElementaryTypeName"
          ? svar.typeName.name
          : svar.typeName.namePath
      }*) **StateVar** *${contract.name}*.**${
        svar.name
      }**${knownValue} ${decl_uri}`,
      decoStyle: decoStyle,
    };
  ...
  static shadowedStateVar(node, document, contract, declaration, prefix) {
    ...
    return {
      ...
      hoverMessage: `${prefix}(*${
        declaration.typeName.type == "ElementaryTypeName"
          ? declaration.typeName.name
          : declaration.typeName.namePath
  ...
  static inheritedStateVar(node, document, contract, declaration) {
    ...
    if (subcontract) {
      ...
      declaration = declaration.hasOwnProperty("_node")
        ? declaration._node
        : declaration;
      decl_uri = `([Declaration: ${subcontract.name}#${declaration.loc.start.line}](${uri}#${declaration.loc.start.line}))`;
      knownType = getVariableDeclarationType(declaration);
    ...
    return {
      ...
      hoverMessage: `${prefix}(*${knownType || "?"}*) **StateVar** *${
        subcontract ? subcontract.name : "Unknown"
      }*.**${node.name}** ${decl_uri}`,
      decoStyle: decoStyle,
    };
  }
...
function init(context) {
  ...
  const decoSuffix =
    settings.extensionConfig().deco.argumentsMode != "color only"
      ? settings.extensionConfig().deco.argumentsSuffix
      : undefined;
  const shouldHighlightArg =
    settings.extensionConfig().deco.argumentsMode != "symbol only";

  for (let idx = 0; idx < 15; idx++) {
    const hue = (((5 + idx) * 19) % 255) / 255;

    const lightBackground = shouldHighlightArg
      ? RGBtoHex(...HSLtoRGB(hue, 0.85, 0.75)) + "50"
      : undefined;

    const darkBackground = shouldHighlightArg
      ? RGBtoHex(...HSLtoRGB((((8 + idx) * 19) % 255) / 255, 0.99, 0.55)) + "30"
      : undefined;

    const afterColor = RGBtoHex(...HSLtoRGB(hue, 0.85, 0.75)) + "95";

    styles[`styleArgument${idx}`] =
  ...
  styles.decoStyleBookmarkGreen = createBookmarkDecorationType(
    gutterIcons.green,
  );
  styles.decoStyleBookmarkBlue = createBookmarkDecorationType(gutterIcons.blue);
  styles.decoStyleBookmarkIssue = createBookmarkDecorationType(
    gutterIcons.issue,
  );
  ...
  const createDecorationType = (options) => {
    ...
    return vscode.window.createTextEditorDecorationType({
      ...
      overviewRulerColor: settings.extensionConfig().overviewruler.decorations
        .enable
        ? borderColor || "blue"
        : undefined,
      overviewRulerLane: settings.extensionConfig().overviewruler.decorations
        .enable

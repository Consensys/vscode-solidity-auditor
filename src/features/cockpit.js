...
function nodeTypeName(node) {
  ...
  return node.typeName.type === "ElementaryTypeName"
    ? node.typeName.name
  ...
  _addPathTree(uri) {
    ...
    for (let idx = 0; idx < pathSegments.length; idx++) {
      ...
      if (!pathObj) {
        ...
        try {
          _type = fs.lstatSync(_abspath).isDirectory()
            ? FilePathTreeDataProvider.TYPE_DIRECTORY
            : FilePathTreeDataProvider.TYPE_FILE;
        } catch (err) {
        ...
        pathObj = {
          ...
          collapsibleState:
            _type === FilePathTreeDataProvider.TYPE_DIRECTORY
      ...
      if (!pathObj) {
        ...
        let _type =
          idx == pathSegments.length - 1
            ? VirtualPathTreeDataProvider.TYPE_LEAF
            : VirtualPathTreeDataProvider.TYPE_NODE;
        pathObj = {
          name: name,
          path: _path,
          label: name,
          ...
          type: _type,
          collapsibleState:
            _type == VirtualPathTreeDataProvider.TYPE_LEAF
              ? 0
              : vscode.TreeItemCollapsibleState.Collapsed,
        };
        parent.push(pathObj);
...
class TopLevelContractsViewDataProvider extends FilePathTreeDataProvider {
  ...
  getTreeItem(element) {
    ...
    ret.iconPath = element.iconPath;
    ret.command =
      element.type === FilePathTreeDataProvider.TYPE_FILE
        ? {
            command: "solidity-va.cockpit.jumpToRange",
            arguments: [element.resource],
            title: "JumpTo",
  ...
  refresh(workspaceRelativeBaseDir) {
    return new Promise((resolve, reject) => {
      this.treeView.cockpit.commands
        ._findTopLevelContracts(undefined, undefined, workspaceRelativeBaseDir)
        .then((data) => {
          this.load(
...
class FTraceViewDataProvider extends BaseDataProvider {
  ...
  dataGetChildren(element) {
    ...
    return Object.keys(element.children).map((k) => {
      let children =
        typeof element.children[k] === "object" ? element.children[k] : {};
      return {
        children: children,
        resource: this.documentUri, //uri
        label: k,
...
class FTraceView extends BaseView {
  ...
  async onDidSelectionChange(event) {
    ...
    let knownFiles = Object.keys(commands.g_workspace.sourceUnits).filter((f) =>
      f.endsWith(".sol"),
    );
  ...
  async onDidSelectionChange(event) {
    ...
    let publicFunctions = currentContract.functions
      .filter((f) => {
        ...
        return (
          !filterNotVisibility.includes(node.visibility) &&
          !filterNotStateMutability.includes(node.stateMutability)
        );
      })
      .reduce((obj, func) => {
        let key = func.name;
  ...
  refresh() {
    return new Promise((resolve, reject) => {
      vscode.workspace
        .findFiles(
          "{**/*.sol}",
          settings.DEFAULT_FINDFILES_EXCLUDES_ALLOWFLAT,
          5000,
        )
        .then((solfiles) => {
          this.load(solfiles);
...
class SettingsViewDataProvider extends VirtualPathTreeDataProvider {
  ...
  getTreeItem(element) {
    let ret = new vscode.TreeItem(
      element.type === VirtualPathTreeDataProvider.TYPE_LEAF
        ? (element.metadata.currentValue === true ? "☑  " : "☐  ") +
          element.label
        : element.label,
  ...
  refresh() {
    return new Promise((resolve, reject) => {
      let settingsState = Object.keys(this.settings).reduce((obj, key) => {
        obj[key] = this.settings[key];
        let k = key.split(".");
        obj[key].extension = k[0];
        obj[key].section = k.slice(1).join(".");
        obj[key].currentValue = vscode.workspace
          .getConfiguration(obj[key].extension)
          .get(obj[key].section);
        return obj;
      }, {});
      ...
      let item = {
        ...
        children: this.data[key].map((element) => {
          return {
            ...
            label: `⇢  ${
              element.call.declaration.name ? element.call.declaration.name : ""
            }.${element.call.name} (${
              nodeTypeName(element.call.declaration) || ""
            })`,
            //iconPath: 0,
            command: {
              command: "solidity-va.cockpit.jumpToRange",
...
class ExtCallView extends BaseView {
  ...
  async onDidSelectionChange(event) {
    ...
    let focusSolidityElement = sourceUnit.getContractAtLocation(
      focus.line,
      focus.character,
    );
    if (!focusSolidityElement) {
...
class Cockpit {
  ...
  async onDidSelectionChange(event) {
    if (
      ...
      event.textEditor.visibleRanges.length <= 0 ||
      !event.selections ||
